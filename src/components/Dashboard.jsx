...
export default function Dashboard() {
  const { transactions, income, expense, setTransactions, deleteTransaction } =
    useTransactions();
      ...
      <main className="flex-1 w-full p-6 mb-40">
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            {/* Category Filter Dropdown with Headless UI */}
            <div className="relative inline-block text-left">
              <Menu>
                <MenuButton
                  className={`w-full text-left pl-12 pr-5 py-4 rounded-2xl border focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-300 backdrop-blur-sm cursor-pointer ${
                    darkMode
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-14 w-full">
                    <span className="flex items-center text-gray-900">
                      {getTransactionIcon(selectedCategory)}
                      {selectedCategory}
                    </span>
                ...
                <MenuItems className="absolute left-0 right-0 z-[9999] mt-2 origin-top-right divide-y divide-gray-100 rounded-xl bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none transform translate-y-0">
                  <div className="px-1 py-1">
                    {categories.map((category) => (
                      <MenuItem key={category}>
                        <button
                          onClick={() => setSelectedCategory(category)}
                          className="cursor-pointer hover:bg-gray-100 hover:text-blue-900 text-gray-900 group flex w-full items-center rounded-lg px-4 py-2 text-sm"
                        >
            ...
            <div
              className={`${
                darkMode
                  ? "flex items-center gap-3 bg-gray-800 backdrop-blur-md px-4 py-2 rounded-xl border border-gray-700"
                  : "flex items-center gap-3 bg-white/70 backdrop-blur-md px-4 py-2 rounded-xl border border-gray-200"
              }`}
            >
              <button
